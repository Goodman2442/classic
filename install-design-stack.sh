...
Порядок работы над интерфейсом:
1. Направление не выбрано, либо результат не нравится и человек не может
   сформулировать чем -> design-council. Если направление ясно, сразу к п.2.
2. Сборка -> design-taste-frontend. Перед кодом объявить Design Read
   и три ручки числами.
3. Посмотреть на то, что вышло -> visual-qa. Это обязательный шаг, а не
   услуга по запросу: без него правки идут вслепую.
4. Нужен глубокий разбор -> /impeccable:critique, правки, затем снова
   visual-qa.
5. В работе есть движение -> числа и кривые у emil-design-eng, финальная
   проверка /review-animations.

Не интерфейс, а презентация, постер, отчёт или письмо: направление и палитру
задаёт design-taste-frontend, дальше работают pptx, theme-factory, dataviz,
canvas-design. Impeccable и GSAP в это не вмешиваются.
